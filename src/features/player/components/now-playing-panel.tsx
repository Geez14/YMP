...
function NowPlayingPanelComponent({ song, textSecondary, textPrimary, accentText, hoverSurface }: NowPlayingPanelProps) {
  return (
    <>
      <div className="flex flex-col items-center gap-5 text-center lg:gap-7">
        <div className="relative w-full max-w-[320px] sm:max-w-[380px]">
          ...
          <div className="relative aspect-square overflow-hidden rounded-[26px] shadow-[0_22px_48px_rgba(54,82,190,0.14)]">
            <Image
              src={song?.cover_path ? API_ROUTES.coverBySongId(song.id) : "/cd_np.svg"}
              alt="Album art"
              width={380}
              height={380}
        ...
        <div className={`grid w-full max-w-[680px] gap-3 ${textPrimary}`}>
          <p className={`text-xs uppercase tracking-[0.3em] ${textSecondary}`}>Now Playing</p>
          ...
          {song ? null : <p className={`text-sm ${textSecondary}`} style={{ marginBottom: "0.15rem" }}></p>}
...
export const NowPlayingPanel = memo(NowPlayingPanelComponent);
